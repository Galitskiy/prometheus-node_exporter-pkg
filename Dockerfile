...
RUN mkdir -p /deb-package/usr/bin /deb-package/var/log/prometheus \
  ...
  && cp node_exporter-$version.linux-amd64/node_exporter /deb-package/usr/bin/$binary_name
...
RUN fpm --output-type deb \
  ...
  --url "${pkg_url}" \
  --deb-systemd "prometheus-node-exporter.service" \
  --deb-default "prometheus-node-exporter" \
  --after-install "after-install.sh" \
  -p ${deb_package_name}-${version_string}.deb \
  etc usr var && cp *.deb /deb-package/
